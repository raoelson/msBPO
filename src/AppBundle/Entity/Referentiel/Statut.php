 ...
 * @ORM\Entity(repositoryClass="AppBundle\Repository\Referentiel\StatutRepository")
...
class Statut
{
     ...
     * @ORM\ManyToMany(targetEntity="TypeAction", mappedBy="statuts", cascade={"persist", "merge"})
     */
    private $typeActions;
    
    /**
     * @var ArrayCollection $operations
     * @ORM\OneToMany(targetEntity="\AppBundle\Entity\Operation", mappedBy="statut", cascade={"persist", "merge"})
     */
    private $operations;
    ...
    public function addTypeAction(\AppBundle\Entity\Referentiel\TypeAction $typeActions)
    {
        if (!$this->typeActions->contains($typeActions)) {
            if (!$typeActions->getStatuts()->contains($this)) {
                $typeActions->addStatuts($this);
            }
